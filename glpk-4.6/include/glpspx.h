...
#if 0
      SPM *AA; /* SPM A[1:m,1:n]; */
      /* constraint matrix (has m rows and n columns) */
#endif
#if 1
      int *aa_ptr; /* int aa_ptr[1+m+n]; */
      /* aa_ptr[0] is not used;
         aa_ptr[i], 1 <= i <= m, is a pointer to the first element of
         i-th row in the arrays aa_ind and aa_val;
         aa_ptr[m+j], 1 <= j <= n, is a pointer to the first element of
         j-th column in the arrays aa_ind and aa_val */
      int *aa_len; /* int aa_len[1+m+n]; */
      /* aa_len[0] is not used;
         aa_len[i], 1 <= i <= m, is the number of (non-zero) elements
         in i-th row;
         aa_len[m+j], 1 <= j <= n, is the number of (non-zero) elements
         in j-th column */
      int *aa_ind; /* int aa_ind[1+nnz+nnz]; */
      /* aa_ind[0] is not used;
         aa_ind[*] are row and column indices of (non-zero) elements */
      double *aa_val; /* double aa_val[1+nnz+nnz]; */
      /* aa_val[0] is not used;
         aa_val[*] are numerical values of (non-zero) elements */
#endif
      /*--------------------------------------------------------------*/
      /* basic solution */
      int b_stat;
