...
{     /* base driver which does not use the presolver */
      SPX _spx, *spx = &_spx;
      int ret;
#if 0
      lpx_to_spx(spx, lp);
#else
{     int i, j, k, m, n, nnz, typx, tagx, loc;
      double rii, sjj, lb, ub;
      spx->m = m = lpx_get_num_rows(lpx);
      spx->n = n = lpx_get_num_cols(lpx);
      spx->typx = ucalloc(1+m+n, sizeof(int));
      spx->lb = ucalloc(1+m+n, sizeof(double));
      spx->ub = ucalloc(1+m+n, sizeof(double));
      for (i = 1; i <= m; i++)
      ...
      spx->coef = ucalloc(1+m+n, sizeof(double));
      ...
      spx->aa_ptr = ucalloc(1+m+n, sizeof(int));
      spx->aa_len = ucalloc(1+m+n, sizeof(int));
      spx->aa_ind = ucalloc(1+nnz+nnz, sizeof(int));
      spx->aa_val = ucalloc(1+nnz+nnz, sizeof(double));
      loc = 1;
      for (i = 1; i <= m; i++)
      {  rii = lpx_get_rii(lpx, i);
         spx->aa_ptr[i] = loc;
         spx->aa_len[i] = lpx_get_mat_row(lpx, i, &spx->aa_ind[loc-1],
            &spx->aa_val[loc-1]);
         for (k = 0; k < spx->aa_len[i]; k++)
         {  j = spx->aa_ind[loc+k];
            spx->aa_val[loc+k] *= (rii * lpx_get_sjj(lpx, j));
         }
         loc += spx->aa_len[i];
      }
      insist(loc == nnz + 1);
      ...
      {  sjj = lpx_get_sjj(lpx, j);
         spx->aa_ptr[m+j] = loc;
         spx->aa_len[m+j] = lpx_get_mat_col(lpx, j, &spx->aa_ind[loc-1],
            &spx->aa_val[loc-1]);
         for (k = 0; k < spx->aa_len[m+j]; k++)
         {  i = spx->aa_ind[loc+k];
            spx->aa_val[loc+k] *= (lpx_get_rii(lpx, i) * sjj);
         }
         loc += spx->aa_len[m+j];
      }
      insist(loc == nnz + nnz + 1);
      ...
      spx->bbar = ucalloc(1+m, sizeof(double));
      spx->pi = ucalloc(1+m, sizeof(double));
      spx->cbar = ucalloc(1+n, sizeof(double));
      spx->some = 0;
...
{     int k, m, n, t;
      ...
      m = spx->m;
      n = spx->n;
      prim = ucalloc(1+m+n, sizeof(double));
      dual = ucalloc(1+m+n, sizeof(double));
      for (k = 1; k <= m+n; k++)
      {  t = spx->posx[k];
         if (t <= m)
         {  prim[k] = spx->bbar[t];
...
{     /* extended driver which uses the presolver */
      LPP *lpp;
      LPX *prob;
...
{     int m = lpx_get_num_rows(lp);
      ...
      A_val = ucalloc(1+n, sizeof(double));
      for (i = 1; i <= m; i++)
      {  /* determine xR[i] */
#if 0 /* 21/XII-2003 */
         if (tagx[i] == LPX_BS)
      ...
      A_val = ucalloc(1+m, sizeof(double));
      for (j = m+1; j <= m+n; j++)
      {  /* determine cS[j] */
#if 0 /* 21/XII-2003 */
         cS_j = coef[j];
