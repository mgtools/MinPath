...
{     LPXCOL *col;
      ...
      if (lp->round && fabs(sum) < 1e-9) sum = 0.0;
      return sum;
}
...
{     double prim;
      ...
      prim = lp->row[i]->prim;
      if (lp->round && fabs(prim) < 1e-9) prim = 0.0;
      return prim;
}
...
{     double dual;
      ...
      dual = lp->row[i]->dual;
      if (lp->round && fabs(dual) < 1e-9) dual = 0.0;
      return dual;
}
...
{     double pval;
      ...
      pval = lp->row[i]->pval;
      if (lp->round && fabs(pval) < 1e-9) pval = 0.0;
      return pval;
}
...
{     double dval;
      ...
      dval = lp->row[i]->dval;
      if (lp->round && fabs(dval) < 1e-9) dval = 0.0;
      return dval;
}
...
{     double mipx;
      ...
      mipx = lp->row[i]->mipx;
      if (lp->round && fabs(mipx) < 1e-9) mipx = 0.0;
      return mipx;
}
...
{     /* determine row mark */
      insist(lp == lp);
      insist(i == i);
      fault("lpx_get_row_mark: no more supported");
      return 0;
}
...
{     /* determine column mark */
      insist(lp == lp);
      insist(j == j);
      fault("lpx_get_col_mark: no more supported");
      return 0;
}
...
{     /* obtain row interior point solution */
      if (vx != NULL) *vx = lpx_ipt_row_prim(lp, i);
      if (dx != NULL) *dx = lpx_ipt_row_dual(lp, i);
      return;
}

void lpx_get_ips_col(LPX *lp, int j, double *vx, double *dx)
{     /* obtain column interior point solution */
      if (vx != NULL) *vx = lpx_ipt_col_prim(lp, j);
      if (dx != NULL) *dx = lpx_ipt_col_dual(lp, j);
      return;
}
